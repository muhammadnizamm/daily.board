...
export function setupCatatan(catatan) {
    ...
    daftarCatatanElement.id = "daftar-catatan";
    catatan.appendChild(daftarCatatanElement);

    function renderCatatan() {
        daftarCatatanElement.innerHTML = "";

        daftarCatatan.forEach(catatanData => {
            const div = document.createElement("div");
            div.className = "catatan-item";

            const p = document.createElement("p");
            p.textContent = catatanData.isi;

            const small = document.createElement("small");
            small.textContent = catatanData.tanggal;
            ...
            tombolHapus.addEventListener("click", () => {
                daftarCatatan = daftarCatatan.filter(item => item.id !== catatanData.id);
            ...
            div.addEventListener("dblclick", () => {
                const isiBaru = prompt("Edit catatan:", catatanData.isi);

                if (isiBaru !== null && validasiCatatan(isiBaru)) {
                    daftarCatatan = daftarCatatan.map(item =>
                        item.id === catatanData.id
                            ? { ...item, isi: isiBaru.trim() }
                            : item
                    );
